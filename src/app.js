...
angular.module('CrimeReport').controller('AppCtrl', function($scope, $http, AppConfig, CrimesData) {
  ...
  $scope.$watchGroup(['model.lat', 'model.lng', 'model.numOfCrimes'], (attrs) => {
    if (attrs[0]) {
      ...
      CrimesData.getCrimes(attrs[0], attrs[1], attrs[2]).then((crimes) => {
        ...
        $scope.model.crimeGroups = _(crimes).groupBy('event_clearance_group').map(function(crimes, groupKey) {
          return {
            type: groupKey,
            count: crimes.length,
            crimes: crimes,
